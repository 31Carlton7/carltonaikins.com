...
If you have been following a lot of tutorials, you’ll quickly realize that they all have very messy ways of organizing their code for the sole purpose of quick development and being able to teach you the topic at hand.

Now in most cases this is fine, but say you are building a large scale project or trying to convert an already existing one; You don’t want to end up putting all of your code in the `/lib` directory! And by wasting too much time figuring this out, you’ll slow yourself down on development and confuse your self on where everything goes.

So in this tutorial we’re going to look at an effective way of organizing your flutter project.

This starts off with your app architecture. This is heavily influenced by what state management tool you decide to use, however many of the practices do remain the same.

This architecture is based on the Riverpod state management tool created by Remi Rousselet so we are going to focus on that for this tutorial.

Riverpod is a Declarative programming for Flutter state management. It allows you to write business logic like Stateless widgets, easily implement ‘pull to refresh’ and more and it enhances compiler, provides lint rules, etc. Learn more at [https://riverpod.dev/](https://riverpod.dev/).
...
Now that we’ve cleared the introductory things out of the way, let’s dive into how we can structure our project! Inside of your terminal run the following command to get your project set up:

```bash
flutter create .
```

Once that’s done we’ll have a folder structure that looks something that looks like this:
...
For the purposes of this tutorial we’re going to focus on the `/lib` directory (where all the magic happens)

### Moving to `/src` folder

The first thing you can go ahead and do is create a folder under the `/lib` directory and call it `src`. This is where all the functionality of the app will live. Inside of `/lib` you only want `main.dart` to remain in here.

## `/config` folder

The first folder we are going to create will be called `config` . In here is going to contain data that will determine the way our app behaves. So general configuration related stuff. You can think of it like the settings screen of an app but this time for your code.

### Constants

So add a new folder inside of `/src` and call it `config` . We are no going to add a couple more folders and files in here. Create a `constants.dart` and `environment_keys.dart` . Fairly self explanatory files. Think of `constants.dart` as a file that simply contains all immutable global variables in your app.
...
Now for the `environment_keys.dart` file, think of it exactly like a `.env` file if you’ve worked with them. It will contain all your public, private, secret keys and other URLs and data. Using `.env` is not a common design pattern in flutter, so we improvise by using a file. Essentially works like the `constants.dart` file in the sense that you’re creating a bunch of immutable values.

Warning: MAKE SURE YOU ADD THIS FILE TO YOUR `.gitignore` . Flutter will not automatically add this because it’s a custom made file so make you do this before committing.

### Themes

Notice how themes is a folder. Incase you want to include dark mode in your app, you would likely want to create 2 separate themes files. In here I also like to keep my typography variables/text theme as well as my colors. Essentially the design system of your app will live here.
...
If you are building an app that requires it to access API endpoints, then it’s definitely a good idea to create a file to house the different types of exceptions/errors that you may encounter throughout your app. It’s usually a good idea to split them up based on the API so that’s why `exceptions` should be housed in a folder.
...
This folder is, as the name suggests, a folder to house all of your models/classes. They will contain methods to convert JSON data to usable classes that you can utilize inside of your app.

Here’s an example of what the folder looks like:
...
And here’s what one of your classes may look like:
...
You can always put in custom methods and values in here. I’d try to avoid this pattern in some cases and do things like `intrContTxt()` inside of your custom backend API unless you’re retrieving the information from another source. Try to make this file as simple as possible whilst having the necessary functions.
...
If you have a custom backend API setup for your app, for example, an express.js server, then you will want to create a `/database_services` folder. In here, for each endpoint, create a file for it and add the functions needed to access each endpoint.
...
It’s also a highly recommended practice to use another external package to perform your GET, POST, PUT, DELETE, etc. operations. The most popular package to do this is [dio](https://pub.dev/packages/dio). Dio is “a powerful HTTP networking package, supports Interceptors, Aborting and canceling a request, Custom adapters, Transformers, etc.”

Here’s my configuration for Dio that you can use in your code as well! It comes with a JSON Parser, Transformer and an Automatic Caching!
...
Now here’s where the real fun is at. So in all of the previous folders, we successfully separated our business logic from our screens. Now we our going to get into our screens and components.

_Note: If you’d like, you can move the `/themes` folder in here._

### Components

We are going to separate everything into 2 folders, `/components` and `/screens` . The `/components` folder will simply house all of the reusable widgets that you use across _multiple_ screens througout your app. You can put custom UI components, replace the default material or cupertino widgets provided by Flutter in here as well!
...
I’d also try to name the components with a naming convention that includes the name of the screen in some way shape or form, whether that’s an abbreviation or the name of it itself, but once again you don’t have to do this.

Another practice, that once again you don’t have to do but will lead to better organization is by having a general view and then subfolders for each view inside of the screen.

For example, let’s say we have a news app. Suppose we tap on an article and then the app performs a GET request to retrieve all of the article’s details. Then on that screen we have a button that goes to a screen that gives “metadata” or extra details about the article that the viewer can see such as exact time it was posted, when the event occured, reporter name and title, etc. This screen and information cannot be found in any other kind of way throughout the app unless you go through the article itself.

In this case you can make a subfolder inside of `/article_views` called `article_views/article_info_view` and in there you can put the view: `article_info_view.dart`

Here’s an example of what the overall structure would look like:
